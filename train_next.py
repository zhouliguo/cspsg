...
from models.yolo_convnext import Model
from utils.autoanchor import check_anchors
from utils.autobatch import check_train_batch_size
from utils.callbacks import Callbacks
from utils.dataloaders import create_dataloader, LoadImagesAndLabels_sg
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        csd = torch.load(weights, map_location='cpu')['model']  # load checkpoint to CPU to avoid CUDA memory leak
        csd_new = OrderedDict()
        for k, v in csd.items():
            k = k.replace('downsample_layers', 'model.0')
            k = k.replace('stages', 'model.1')
            csd_new[k] = v
        model = Model(cfg, ch=3, nc=nc).to(device)  # create
        exclude = ['anchor'] if (cfg or hyp.get('anchors')) and not resume else []  # exclude keys
        #csd = ckpt.state_dict()  # checkpoint state_dict as FP32
        csd = intersect_dicts(csd_new, model.state_dict(), exclude=exclude)  # intersect
    ...
    # Trainloader
    train_dataset = LoadImagesAndLabels_sg(train_path, phase='train', size=imgsz, hyp=hyp)
    ...
    val_dataset = LoadImagesAndLabels_sg(val_path, phase='val')
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, labels, paths) in pbar:  # batch -------------------------------------------------------------
            ...
            if RANK in {-1, 0}:
                ...
                callbacks.run('on_train_batch_end', ni, model, imgs, labels, paths, plots=False)
...
def parse_opt(known=False):
    ...
    #parser.add_argument('--weights', type=str, default=ROOT / 'convnext_base_22k_224.pt', help='initial weights path')
