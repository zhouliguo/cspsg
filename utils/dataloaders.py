...
def img2label_paths(img_paths):
    ...
    return [sb.join(x.rsplit(sa, 1)).rsplit('_', 1)[0] + '_gtFine_labelTrainIds.png' for x in img_paths]

def img2label_paths_c(img_paths):
    # Define label paths as a function of image paths
    sa, sb = f'{os.sep}leftImg8bit{os.sep}', f'{os.sep}gtFine{os.sep}'  # /images/, /labels/ substrings
...
class LoadImagesAndLabels_sg(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.phase == 'train':
            ...
            img_w = int(image.size[0]*reratio/4)*4
            img_h = int(image.size[1]*reratio/4)*4
            ...
            if img_h<self.h:
                ...
                image_new.paste(image, (0, (self.h-img_h)//2))
                label_new.paste(label, (0, (self.h-img_h)//2))
...
class LoadImagesAndLabels_sg_fc(Dataset):
    '''
    phase: 'train', 'val' or 'test'. 
           'train' include data augmentation, 
           'test' return an additional flipped image.
    ...
    def __getitem__(self, index):
        ...
        if self.phase == 'test':
            ...
            return image, label, image_f

        return image, label, self.image_path[index]

class LoadImagesAndLabels_sg_mosaic(Dataset):
    '''
    phase: 'train', 'val' or 'test'. 
           'train' include data augmentation, 
           'test' return an additional flipped image.
    ...
    def __init__(self, path, phase, size=None, hyp=None):
        ...
        #self.mean = torch.as_tensor([0.485, 0.456, 0.406])
        #self.std = torch.as_tensor([0.229, 0.224, 0.225])
        self.indices = range(len(self.image_path))
    ...
    def __getitem__(self, index):
        if self.phase == 'train':
            mosaic_im = np.zeros((2*self.h, 2*self.w, 3), np.uint8)
            mosaic_lb = np.zeros((2*self.h, 2*self.w), np.uint8)
            ...
            for k, index in enumerate(indices):
                ...
                # Random crop
                image = np.array(transformsF.crop(image, i, j, h, w), np.uint8)
                label = np.array(transformsF.crop(label, i, j, h, w), np.uint8)
                ...
                if k == 0:
                    mosaic_im[0:self.h, 0:self.w] = image
                    mosaic_lb[0:self.h, 0:self.w] = label
                if k == 1:
                    mosaic_im[self.h:2*self.h, 0:self.w] = image
                    mosaic_lb[self.h:2*self.h, 0:self.w] = label
                if k == 2:
                    mosaic_im[0:self.h, self.w:2*self.w] = image
                    mosaic_lb[0:self.h, self.w:2*self.w] = label
                if k == 3:
                    mosaic_im[self.h:2*self.h, self.w:2*self.w] = image
                    mosaic_lb[self.h:2*self.h, self.w:2*self.w] = label

                #cv2.imshow('mosaic_im', mosaic_im)
                #cv2.imshow('mosaic_lb', mosaic_lb)
                #cv2.waitKey()

            #i, j, h, w = transforms.RandomCrop.get_params(Image.fromarray(mosaic_im), (self.h, self.w))
            image = mosaic_im #[i:i+h, j:j+w]
            label = mosaic_lb #[i:i+h, j:j+w]

            label_19 = np.zeros((label.shape[0], label.shape[1], 19), np.float32)
            for k in range(19):
                index_cls = np.where(label==k)
                if len(index_cls[0])!=0:
                    label_19[index_cls[0], index_cls[1], k] = 1
