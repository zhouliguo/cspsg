...
from models.yolo_cls import Model
from utils.autoanchor import check_anchors
from utils.autobatch import check_train_batch_size
from utils.callbacks import Callbacks
from utils.dataloaders import create_dataloader, LoadImagesAndLabels_cls
...
from utils.loss_cls import ComputeLoss
from utils.metrics import fitness
from utils.plots import plot_evolve, plot_labels
from utils.torch_utils import EarlyStopping, ModelEMA, de_parallel, select_device, torch_distributed_zero_first
from utils.acc import AverageMeter, Summary, accuracy
import warnings
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Trainloader
    train_dataset = datasets.ImageFolder(train_path, transforms.Compose([transforms.RandomResizedCrop(imgsz),transforms.RandomHorizontalFlip(),transforms.ToTensor()]))
    ...
    val_dataset = datasets.ImageFolder(val_path, transforms.Compose([transforms.Resize(256),transforms.CenterCrop(imgsz),transforms.ToTensor()]))
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=2*batch_size, shuffle=False, num_workers=workers, pin_memory=True)
    ...
    #hyp['cls'] *= nc / 80 * 1 / nl  # scale to classes and layers
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, labels) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    labels = nn.functional.interpolate(labels, size=ns, mode='nearest')
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                loss, loss_items = compute_loss(pred, labels.to(device))  # loss scaled by batch_size
                if RANK != -1:
            ...
            if RANK in {-1, 0}:
                ...
                pbar.set_description(('%10s' * 2 + '%10.4g' * 5) %
                                     (f'{epoch}/{epochs - 1}', mem, *mloss, labels.shape[0], imgs.shape[-1]))
                callbacks.run('on_train_batch_end', ni, model, imgs, labels, paths=None, plots=False)
        ...
        with torch.no_grad():
            ...
            for i, (imgs, labels) in enumerate(val_loader):  # batch -------------------------------------------------------------
                imgs = imgs.to(device, non_blocking=True)
                labels = labels.to(device, non_blocking=True)
                preds = model(imgs)[0][:,0,0]
            ...
            if accuracy_max < acc:
                ...
                accuracy_max = acc
            print('Accuracy:', acc)
            f_acc.write('Accuracy:'+str(accuracy_max)+'\n')
            f_acc.flush()
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='yolov5-cls.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/imagenet.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.imagenet.yaml', help='hyperparameters path')
